...
class IncrementalClassifier(nn.Module):
    ...
    def forward(self, x):
        #print(x.shape)  #torch.Size([5, 100, 256])
        if self.norm_feat:
        ...
        #print(torch.cat(out, dim=2).shape)  #torch.Size([5, 100, 31])
        return torch.cat(out, dim=2)  ###############   original = 2



class IncrementalClassifier_Video(nn.Module):
    def __init__(self, classes, norm_feat=False, channels=256, bias=True, deep_cls=False):
        super().__init__()
    ...
    def forward(self, x):
        #print(x.shape)
        #IncrementalClassifier_Video->x <class 'torch.Tensor'> torch.Size([3, 3, 1, 10, 256])
        if self.norm_feat:
            x = F.normalize(x, p=2, dim=3)
        out = []
        for mod in self.cls[1:]:
            out.append(mod(x))
        out.append(self.cls[0](x))  # put as last the void class
        #print(torch.cat(out, dim=4).shape)  #
        #IncrementalClassifier_Video->torch.cat(out, dim=4).shape torch.Size([3, 3, 1, 10, 26])
        return torch.cat(out, dim=4)  ###############   original = 2
...
class CosineClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.cat(out, dim=3)  ###############   original = 2
