...
__all__ = ["load_ytvis_json_val", "register_ytvis_instances_val"]
...
def load_ytvis_json_val(json_file, image_root, dataset_name=None, extra_annotation_keys=None, cfg=None, train=False):
    ...
    if dataset_name is not None:
        ...
        #print("ytvis_valcat_ids", cat_ids) ###########
        cats = ytvis_api.loadCats(cat_ids)
        # The categories in a custom json file may not be sorted.
        thing_classes = [c["name"] for c in sorted(cats, key=lambda x: x["id"])]
        #print("ytvis_val_thing_classes", thing_classes)  ###################
...
"""
    ...
    for (vid_dict, anno_dict_list) in vids_anns:
        ...
        for base_classes_anno in anno_dict_list:
            ...
            else:   ##    cfg.CONT.BASE_CLS + (cfg.CONT.TASK - 1) * cfg.CONT.INC_CLS <=
                if (base_classes_anno["category_id"]-1) < cfg.CONT.BASE_CLS + cfg.CONT.TASK * cfg.CONT.INC_CLS:
                        base_classes_dataset.append(base_classes_anno)
...
def register_ytvis_instances_val(name, metadata, json_file, image_root, cfg=None, train=False,):
    """
    ...
    DatasetCatalog.register(name, lambda: load_ytvis_json_val(json_file, image_root, name, cfg=cfg, train=train))
...
if __name__ == "__main__":
    ...
    dicts = load_ytvis_json_val(json_file, image_root, dataset_name="ytvis_2019_train")
