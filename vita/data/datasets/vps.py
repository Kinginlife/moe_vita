...
def load_video_vspw_vps_json(json_file, image_dir, gt_dir, cfg=None, train=False):
    """
    Args:
    ...
    for ann in json_info["annotations"]:
        ...
        for image in anns:
            ...
            #################################################################################################
            segments_info = []
            for ins in image["segments_info"]:
                if cfg.CONT.TASK == 0:
                    if ins["category_id"] in stuff_ids_to_continue_dic:
                        if stuff_ids_to_continue_dic[ins["category_id"]] < cfg.CONT.BASE_CLS:
                            segments_info.append(ins)
                    else:
                        if thing_ids_to_continue_dic[ins["category_id"]] < cfg.CONT.BASE_CLS:
                            segments_info.append(ins)
                    #if (ins["category_id"]) < cfg.CONT.BASE_CLS:
                        #segments_info.append(ins)
                else:
                    if ins["category_id"] in stuff_ids_to_continue_dic:
                        if cfg.CONT.BASE_CLS + (cfg.CONT.TASK - 1) * cfg.CONT.INC_CLS <= stuff_ids_to_continue_dic[ins["category_id"]] < cfg.CONT.BASE_CLS + cfg.CONT.TASK * cfg.CONT.INC_CLS:
                            segments_info.append(ins)
                    else:
                        if cfg.CONT.BASE_CLS + (cfg.CONT.TASK - 1) * cfg.CONT.INC_CLS <= thing_ids_to_continue_dic[ins["category_id"]] < cfg.CONT.BASE_CLS + cfg.CONT.TASK * cfg.CONT.INC_CLS:
                            segments_info.append(ins)

                    #if cfg.CONT.BASE_CLS + (cfg.CONT.TASK - 1) * cfg.CONT.INC_CLS <= (ins["category_id"]) < cfg.CONT.BASE_CLS + cfg.CONT.TASK * cfg.CONT.INC_CLS:
                        #segments_info.append(ins)
...
def register_video_vspw_vps_json(
        name, metadata, image_root, panoptic_root, panoptic_json, instances_json=None, cfg=None, train=False,
):
    """
    ...
    DatasetCatalog.register(
        ...
        lambda: load_video_vspw_vps_json(
            panoptic_json, image_root, panoptic_root, cfg=cfg, train=train
        ),
    )
...
_PREDEFINED_SPLITS_PANOVSPW = {
    "panoVSPW_vps_video_train": (
        "VIPSeg_720P/images",
        "VIPSeg_720P/panomasksRGB",
        "VIPSeg_720P/panoptic_gt_VIPSeg_train.json",
    ),
    "panoVSPW_vps_video_val": (
        "VIPSeg_720P/images",
        "VIPSeg_720P/panomasksRGB",
        "VIPSeg_720P/panoptic_gt_VIPSeg_val.json",
    ),
    "panoVSPW_vps_video_test": (
        "VIPSeg_720P/images",
        "VIPSeg_720P/panomasksRGB",
        "VIPSeg_720P/panoptic_gt_VIPSeg_test.json",
    ),
}


def register_all_video_panoVSPW(root, cfg, train=False):
    for (
            prefix,
