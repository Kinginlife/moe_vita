...
__all__ = ["load_ytvis_json", "register_ytvis_instances_mem"]
...
"""
    ...
    for (vid_dict, anno_dict_list) in vids_anns:
        ...
        #############################################################
        if not train:  # 这里是 not train，与普通版本相反！
            base_classes_dataset = []
            task = cfg.CONT.TASK-1  #上一个任务（用于特征提取）
            for base_classes_anno in anno_dict_list:
                if task == 0: #第一个任务只保留基础类别
                    if (base_classes_anno["category_id"]-1) < cfg.CONT.BASE_CLS:
                        base_classes_dataset.append(base_classes_anno)
                else: #后续任务只保留上一个任务的类别
                    if cfg.CONT.BASE_CLS + (task - 1) * cfg.CONT.INC_CLS <= (base_classes_anno["category_id"]-1) < cfg.CONT.BASE_CLS + task * cfg.CONT.INC_CLS:
                        base_classes_dataset.append(base_classes_anno)
...
def register_ytvis_instances_mem(name, metadata, json_file, image_root, cfg=None, train=False,):
    """
