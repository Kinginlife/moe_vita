...
class YTVISEvaluator(DatasetEvaluator):
    ...
    def _eval_predictions(self, predictions):
        ...
        # Run evaluation if annotations are available
        if len(self._ytvis_api.dataset.get('annotations', [])) > 0:
            self._do_evaluation(predictions)
        else:
            self._logger.info("Annotations are not available for evaluation.")
        return

    def _do_evaluation(self, predictions):
        """Run YTVIS evaluation and save results."""
        from .datasets.ytvis_api.ytvoseval import YTVOSeval

        ytvis_results = self._ytvis_api.loadRes(predictions)
        ytvis_eval = YTVOSeval(self._ytvis_api, ytvis_results)
        ytvis_eval.evaluate()
        ...
        if self._output_dir:
            ...
            with open(file_path_txt, 'a') as f:
                for idx, stat in enumerate(ytvis_eval.stats):
                    f.write(f"Stat[{idx}]: {stat}\n")
                f.write("\n")
            self._logger.info(f"Saved evaluation results to {file_path_txt}")

        # Store results
        self._results["segm"] = {
            "AP": ytvis_eval.stats[0],
            "AP50": ytvis_eval.stats[1],
            "AP75": ytvis_eval.stats[2],
            "AR1": ytvis_eval.stats[6],
            "AR10": ytvis_eval.stats[7],
        }

    def eval_incremental_per_task(self, predictions_list, num_tasks):
        """Evaluate per-task performance for continual learning."""
        ...
        self._logger.info(f"Saved per-task evaluation results to {file_path_txt}")
