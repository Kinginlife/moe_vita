...
class PanopticDatasetVideoMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        for insid_, class_id_ in insid_catid_dic.items():
            if class_id_ not in self.stuff_ids_to_continue_dic:
                if insid_ in unique_ids:
                    classes.append(self.thing_ids_to_continue_dic[class_id_] + len(self.stuff_ids_to_continue_dic))
                    masks.append(input_panoptic_seg == insid_)
                continue

            if insid_ in unique_ids:
                classes.append(self.stuff_ids_to_continue_dic[class_id_])
                masks.append(input_panoptic_seg == insid_)
