...
class YTVOS_incremental_per_task:
    ...
    def createIndex(self, cfg):
            ...
            if self.dataset['annotations'] != None:   ###########################################
                for ann in self.dataset['annotations']:
                    if cfg.CONT.TASK == 0:
                        ...
                        if cfg.CONT.BASE_CLS + (cfg.CONT.TASK - 1) * cfg.CONT.INC_CLS <= (ann['category_id'] - 1) < cfg.CONT.BASE_CLS + cfg.CONT.TASK * cfg.CONT.INC_CLS:
                            vidToAnns[ann['video_id']].append(ann)
                            anns[ann['id']] = ann
        ...
        if 'categories' in self.dataset:
            for cat in self.dataset['categories']:
                if cfg.CONT.TASK == 0:
                    ...
                    if cfg.CONT.BASE_CLS + (cfg.CONT.TASK - 1) * cfg.CONT.INC_CLS <= (cat['id'] - 1) < cfg.CONT.BASE_CLS + cfg.CONT.TASK * cfg.CONT.INC_CLS:
                        cats[cat['id']] = cat
        ...
        ##########################################################
        self.vidid_filter = vidid_filter
        self.catsIds_filter = catsIds_filter
        self.annIds_filter = annIds_filter
        ##########################################################
    ...
    def loadRes(self, resFile, cfg):
        ...
        #print(res.dataset['videos']) #########

        print('Loading and preparing results...')
        tic = time.time()
        if type(resFile) == str or (PYTHON_VERSION == 2 and type(resFile) == unicode):
            anns_old = json.load(open(resFile))
            #print('1')  ################ true
        elif type(resFile) == np.ndarray:
            anns_old = self.loadNumpyAnnotations(resFile)
            #print('2')
        else:
            anns_old = resFile
            #print('3')
        assert type(anns_old) == list, 'results in not an array of objects'
        ...
        anns = []
        for ann in anns_old:
            if ann['video_id'] in self.vidid_filter:
                annsVidIds.append(ann['video_id'])
                anns.append(ann) #########################################
