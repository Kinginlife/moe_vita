...
def filter_empty_instances(instances, by_box=True, by_mask=True, box_threshold=1e-5):  #移除没有有效 box/mask/类别的实例
    """
...
def _get_dummy_anno(): #生成一个占位标注，确保在按 id 排序后，未被真实标注覆盖的位置也能填上默认结构。
    return {
        "iscrowd": 0,
        "category_id": -1,
        "id": -1,
...
def ytvis_annotations_to_instances(annos, image_size):  #把 YTVIS 格式的锚点（含 bbox、segmentation、id 等）转换成 Detectron2 的 Instances，方便模型读取。
    """
...
def convert_coco_poly_to_mask(segmentations, height, width): #COCO polygon → 二值 mask
    masks = []
...
class YTVISDatasetMapper:
    ...
    def from_config(cls, cfg, is_train: bool = True, is_tgt: bool = True):
        ...
        ret = {
            ...
            "tgt_dataset_name": cfg.DATASETS.TRAIN_MEM[-1],
        }
...
#采样逻辑
        ...
        if self.is_train:  ###########################################
            ref_frame = random.randrange(video_length)#随机选择一个帧
...
class CocoClipDatasetMapper:
    ...
    def __init__(
        ...
        *,
        ...
        if not is_tgt:
            ...
            if tgt_dataset_name.startswith("mem_ytvis_2019"):
                src2tgt = COCO_TO_YTVIS_2019
            elif tgt_dataset_name.startswith("mem_ytvis_2021"):
                src2tgt = COCO_TO_YTVIS_2021
            elif tgt_dataset_name.startswith("mem_ovis"):
    ...
    def from_config(cls, cfg, is_train: bool = True, is_tgt: bool = True):
        if is_tgt:
            augs = build_augmentation(cfg, is_train)  #vita/data/augmentation.py  433
        else:
