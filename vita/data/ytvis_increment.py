"""
YTVIS dataset loader with incremental learning support for VITA.
Adapted from HVPL hvpl/data/datasets/ytvis.py
"""
import contextlib
import io
import json
...
__all__ = ["load_ytvis_json_incremental", "register_ytvis_instances_incremental",
           "COCO_TO_YTVIS_2019", "COCO_TO_YTVIS_2021",
           "YTVIS_CATEGORIES_2019", "YTVIS_CATEGORIES_2021"]

# COCO to YTVIS 2019 category mapping
COCO_TO_YTVIS_2019 = {
    1:1, 2:21, 3:6, 4:21, 5:28, 7:17, 8:29, 9:34, 17:14, 18:8, 19:18,
    21:15, 22:32, 23:20, 24:30, 25:22, 35:33, 36:33, 41:5, 42:27, 43:40
}

# COCO to YTVIS 2021 category mapping
COCO_TO_YTVIS_2021 = {
    1:26, 2:23, 3:5, 4:23, 5:1, 7:36, 8:37, 9:4, 16:3, 17:6, 18:9, 19:19,
    21:7, 22:12, 23:2, 24:40, 25:18, 34:14, 35:31, 36:31, 41:29, 42:33, 43:34
}
...
def load_ytvis_json_incremental(json_file, image_root, dataset_name=None, extra_annotation_keys=None, cfg=None, train=False):
    """
    Load YTVIS json with incremental learning class filtering.

    Args:
        json_file: Path to YTVIS json file
        image_root: Directory containing video frames
        dataset_name: Name of the dataset
        extra_annotation_keys: Additional annotation keys
        cfg: Config object with CONT settings
    ...
    for (vid_dict, anno_dict_list) in vids_anns:
        ...
        # Incremental learning: filter annotations by task
        if train:
            base_classes_dataset = []
            for anno in anno_dict_list:
                category_id_0indexed = anno["category_id"] - 1  # Convert to 0-indexed

                if cfg.CONT.TASK == 0:
                    # Base task: only include base classes
                    if category_id_0indexed < cfg.CONT.BASE_CLS:
                        base_classes_dataset.append(anno)
                else:
                    ...
                    if task_start <= category_id_0indexed < task_end:
                        base_classes_dataset.append(anno)

            if len(base_classes_dataset) == 0:
                continue
        else:
            base_classes_dataset = anno_dict_list

        video_objs = []
        for frame_idx in range(record["length"]):
            frame_objs = []
            for anno in base_classes_dataset:
    ...
    logger.info(f"Filtered to {len(dataset_dicts)} videos for task {cfg.CONT.TASK}")
    return dataset_dicts


def register_ytvis_instances_incremental(name, metadata, json_file, image_root, cfg=None, train=False):
    """
    Register a YTVIS dataset with incremental learning support.
    """
    ...
    DatasetCatalog.register(
        name,
        lambda: load_ytvis_json_incremental(json_file, image_root, name, cfg=cfg, train=train)
    )
