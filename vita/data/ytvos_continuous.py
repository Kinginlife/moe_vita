...
class YTVOS_incremental:
    def __init__(self, annotation_file=None, cfg=None):
        """
        ...
        self.vidid_filter, self.catsIds_filter, self.annIds_filter = [], [], []
        ...
        if not annotation_file == None:
            ...
            self.createIndex(cfg)
#根据当前任务进度 ，从原始的 YTVIS 标注数据中筛选出“当前应参与评估的”标注、视频和类别，并建立一组便于评估的索引/映射结构
    def createIndex(self, cfg):
        # create index
        print('creating index...')
        anns, cats, vids = {}, {}, {}
        vidToAnns,catToVids = defaultdict(list),defaultdict(list)
        vidid_filter, catsIds_filter, annIds_filter = [], [], []  ################
        if 'annotations' in self.dataset:
            if self.dataset['annotations'] != None:   ###########################################
                for ann in self.dataset['annotations']:
                    if cfg.CONT.TASK == 0:
                        if (ann["category_id"] - 1) < cfg.CONT.BASE_CLS:
                            vidToAnns[ann['video_id']].append(ann)
                            anns[ann['id']] = ann

                            if ann['video_id'] not in vidid_filter:
                                vidid_filter.append(ann['video_id'])

                            if ann['id'] not in annIds_filter:
                                annIds_filter.append(ann['id'])
                    else:
                        if (ann['category_id'] - 1) < cfg.CONT.BASE_CLS + cfg.CONT.TASK * cfg.CONT.INC_CLS:
                            vidToAnns[ann['video_id']].append(ann)
                            anns[ann['id']] = ann
        ...
        if 'videos' in self.dataset:
            for vid in self.dataset['videos']:
                if vid['id'] in vidid_filter:  #############################################
                    vids[vid['id']] = vid

        if 'categories' in self.dataset:
            for cat in self.dataset['categories']:
                if cfg.CONT.TASK == 0:
                    if (cat["id"] - 1) < cfg.CONT.BASE_CLS:
                        cats[cat['id']] = cat

                        if cat['id'] not in catsIds_filter:
                            catsIds_filter.append(cat['id'])
                else:
                    if (cat['id'] - 1) < cfg.CONT.BASE_CLS + cfg.CONT.TASK * cfg.CONT.INC_CLS:
                        cats[cat['id']] = cat
        ...
        self.anns = anns #dict[int -> dict]键为 annotation_id，值为该条标注的完整字典（仅保留当前任务允许的类别范围内的标注）
        self.vidToAnns = vidToAnns  #dict[int -> list[dict]] 键为 video_id，值为该视频下所有“被保留”的标注列表
        self.catToVids = catToVids #键为 category_id，值为包含该类别标注的 video_id 列表（
        self.vids = vids #键为 video_id，值为该视频的元数据（如 height、width、length、file_names、id 等
        self.cats = cats  #键为 category_id，值为该类别的元数据（id、name、color 等）

        ##########################################################
        self.vidid_filter = vidid_filter #list[int]  通过筛选的视频 id 列表（含有被保留标注的视频）。评估时会用它来限制参与评估的 video 集合
        self.catsIds_filter = catsIds_filter #list[int]  通过筛选的类别 id 列表（含有被保留类别的标注）。评估时会用它来限制参与评估的类别集合
        self.annIds_filter = annIds_filter #list[int]  通过筛选的标注 id 列表（含有被保留标注的 annotation）。评估时会用它来限制参与评估的 annotation 集合
        ##########################################################
